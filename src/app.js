import { animations, getAnimations, getCategories, getEasings, defaultSettings, settingsStyle, demoClass, demoCSS, generateCode, filterAnimations } from './catalog.js';
import { icon, logo } from './icons.js';
import { getLocale, setLocale, supportedLocales, localeNames, t } from './i18n.js';
...
function renderShell() {
  ...
  previewStyle.textContent = items.map(a => interactivePreviewCSS(a.css)).join('\n');
    ...
    <main id="main">
      ...
      <footer class="main-footer">
        ...
        <div class="developer-info" aria-label="${tr('developerInfo')}">
          <div class="developer-credit"><span class="developer-label">${tr('developer')}</span><strong lang="zh-Hant">交給我科技工作室</strong><span class="developer-english" lang="en">handlebyme.tech</span></div>
  ...
  $('.sidebar').inert = mobileLayout.matches && !document.body.classList.contains('menu-open');
  $('.mobile-menu').setAttribute('aria-expanded', document.body.classList.contains('menu-open'));
  renderCards();
}
...
function renderCards() {
  ...
  $('#result-label').textContent = state.query || state.category !== 'all' || state.favoritesOnly ? t(items.length === 1 ? 'resultCountOne' : 'resultCount', { count: items.length }) + (state.query ? t('resultSearch', { query: state.query }) : '') : t('resultDefault');
  $('#favorite-count').textContent = state.favorites.length;
  $('#library-title').firstChild.textContent = state.favoritesOnly ? t('favoritesTitle') : state.category === 'all' ? t('collectionTitle') : categories.find(c => c.id === state.category).name;
  $('#library-description').textContent = state.favoritesOnly ? t('favoritesDescription') : t('collectionDescription');
  $('#breadcrumb-current').textContent = state.favoritesOnly ? t('favorites') : categories.find(c => c.id === state.category).name;
...
function toggleFavorite(id) {
  ...
  toast(t(state.favorites.includes(id) ? 'favoriteAdded' : 'favoriteRemoved') + (persisted ? '' : t('sessionOnly')));
}
...
function updateEditorFavorite() {
  ...
  $('#editor-favorite').setAttribute('aria-label', t(saved ? 'unsaveItem' : 'saveItem', { name: activeItem.name }));
}
...
function updateEditorCode() {
  ...
  $('#code-help').textContent = { html: t('helpHtml'), css: t('helpCss'), react: t('helpReact'), vue: t('helpVue') }[activeFormat];
...
document.addEventListener('click', async event => {
  ...
  if (action === 'random') { const candidates = getAnimations().filter(a => a.id !== activeItem?.id); setMenu(false); openEditor(candidates[Math.floor(Math.random() * candidates.length)].id); }
  if (action === 'close-editor') closeDialog('editor');
  if (action === 'theme') { state.theme = state.theme === 'light' ? 'dark' : 'light'; save('pufficss-theme', state.theme); document.documentElement.dataset.theme = state.theme; trigger.innerHTML = icon(state.theme === 'dark' ? 'sun' : 'moon'); trigger.setAttribute('aria-label', t(state.theme === 'dark' ? 'lightTheme' : 'darkTheme')); }
  if (action === 'pause') { state.paused = !state.paused; $('#animation-grid').classList.toggle('is-paused', state.paused); trigger.innerHTML = `${icon(state.paused ? 'play' : 'pause')}<span>${tr(state.paused ? 'playAnimations' : 'pauseAnimations')}</span>`; trigger.setAttribute('aria-pressed', state.paused); }
  if (action === 'replay') { const demo = $(`[data-id="${id}"] .demo-content`); const item = getAnimations().find(a => a.id === id); if (item.trigger === 'hover') replayHover(demo, item.duration); else demo.getAnimations({ subtree: true }).forEach(a => { a.currentTime = 0; }); }
  ...
  if (action === 'editor-pause') { editorPaused = !editorPaused; $('#editor-preview').classList.toggle('is-paused', editorPaused); trigger.innerHTML = icon(editorPaused ? 'play' : 'pause'); trigger.setAttribute('aria-label', t(editorPaused ? 'playPreview' : 'pausePreview')); trigger.setAttribute('aria-pressed', editorPaused); }
  if (action === 'reset-settings') { activeSettings = defaultSettings(activeItem); $('#duration').value = activeSettings.duration; $('#duration-output').textContent = `${activeSettings.duration}s`; $('#delay').value = 0; $('#delay-output').textContent = '0s'; $('#easing').value = activeSettings.easing; $('#accent').value = activeSettings.accent; $('#accent-output').textContent = activeSettings.accent; if ($('#loop')) $('#loop').checked = true; updateEditorPreview(); updateEditorCode(); toast(t('settingsReset')); }
  if (action === 'quick-copy') { const item = getAnimations().find(a => a.id === id); if (await copyText(generateCode(item))) toast(t('quickCopied', { name: item.name })); }
  if (action === 'copy') { if (await copyText(generateCode(activeItem, activeSettings, activeFormat))) toast(t('codeCopied')); }
  if (action === 'download') { const code = generateCode(activeItem, activeSettings, activeFormat); const blob = new Blob([code], { type: activeFormat === 'html' ? 'text/html;charset=utf-8' : 'text/plain;charset=utf-8' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = { html: `${activeItem.id}.html`, css: `${activeItem.id}.css`, react: 'MotionDemo.jsx', vue: 'MotionDemo.vue' }[activeFormat]; a.click(); setTimeout(() => URL.revokeObjectURL(url), 1000); toast(t('codeDownloaded')); }
});

document.addEventListener('input', event => {
  if (event.target.id === 'search') { state.query = event.target.value; renderCards(); }
});
document.addEventListener('change', event => {
  if (event.target.id === 'sort') { state.sort = event.target.value; renderCards(); }
  if (event.target.id === 'language-select') {
    const persisted = setLocale(event.target.value);
    toast(t('languageChanged', { language: localeNames[getLocale()] }) + (persisted ? '' : t('sessionOnly')));
  }
});
document.addEventListener('pufficss:localechange', () => {
  ...
  if (guideOpen) { renderGuide(); $('#guide').scrollTop = guideScroll; }
  const focusRoot = editorOpen ? $('#editor') : guideOpen ? $('#guide') : document;
  const focusTarget = focusId ? focusRoot.querySelector('#' + CSS.escape(focusId))
    : focusAction ? [...focusRoot.querySelectorAll('[data-action]')].find(element => element.dataset.action === focusAction && element.dataset.id === focusItem)
    : menuOpen ? $('.brand') : null;
